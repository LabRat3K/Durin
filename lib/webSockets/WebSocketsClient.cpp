...
void WebSocketsClient::loop(void) {
	...
	// Work around websocket SSL cert fingerprint check fail by ignoring fingerprint check.
	// This could allow a well-funded adversary in a privileged position on the network (ISP, nation-state)
	// to perform a MITM attack against this slackbot. For my use-case, this is a non-issue. It should go without saying,
	// but don't use this slackbot to control anything that could harm living things.
	// Hopefully this issue is fixed upstream, in the esp8266-arduino library. It could be worked around by manually
	// getting the websocket SSL fingerprint, like we do for the HTTP SSL cert. I'm not sure if it is the same cert or different,
	// and how often it changes, so for now this is an acceptable trade-off
	//
...
// LabRat Hack
        if (!_fingerprint.compareTo(_host)) {
        //if(!_client.ssl->verify(_fingerprint.c_str(), _host.c_str())) {
            DEBUG_WEBSOCKETS("[WS-Client] certificate mismatch\n");
            DEBUG_WEBSOCKETS(_fingerprint.c_str());
            DEBUG_WEBSOCKETS(" vs ");       
